...
//ROBOT STARTS WORKING USING ARDUINO LOOP FUNCTION
void loop()
{
    detectdistance(); // start detecting objects nearby
    left(); //robot rotates continuously on its place and keep searching for object 
    if (distance <= 40) // ultrasonic sensor detects any object within range of 40 cm
    {
        forward(); // robot moves towards the object
        detectdistance(); //keep detecting distance
        if (distance > 3 && distance <= 13) // if object is 13cm away
        {
            halt(); // bot stops
            delay(1000);
            detectcolor(); // starts to detect color using tcs 3200
            delay(5000);   // wait for 5 second to detect color
            if (frequency1 > 30 && frequency1 < 270) // settting condition for checking if object has RED color
            {
                ...
                delay(2000);     
                //  As red colored object is detected now bot goes backward to original location
                // bot reverses
                backward();
